...
let csvFilepath = __dirname + '/data/title.basics.csv';
let csvHeaders = 'movieId:ID,title:string,start:int,end:int,runtime:int,genres:string[],:LABEL \n';
    ...
    http.get(url, response => {
        ...
        lineReader.on('line', line => {
            if (n == 0) {
                ...
                let title = values[2].replace(/"/g, '""');
                let start = values[5].replace('\\N', ' ');
                let end = values[6].replace('\\N', ' ');
                let runtime = values[7].replace('\\N', ' ');
                let genres = values[8].replace('\\N', ' ').replace(/,/g, ';');

                stream.write(`${movieID},"${title}",${start},${end},${runtime},${genres},Movie \n`);
            }
            n++
        });
